...
if ( ! class_exists( 'WPMOLYP_Module' ) ) {
	...
	abstract class WPMOLYP_Module {
		 ...
		 * 
		 * Simple alias of WPMOLYP_Module::render_template() adding admin
		...
		public static function render_template( $default_template_path = false, $variables = array(), $require = 'once', $admin = false ) {
			...
			do_action( "wpmolyp_render_{$admin}template_pre", $default_template_path, $variables );

			$template_path = locate_template( 'wpmovielibrary/' . $default_template_path, false, false );
			if ( ! $template_path )
				$template_path = WPMOLYP_PATH . '/views/' . $default_template_path;

			$template_path = apply_filters( "wpmolyp_{$admin}template_path", $template_path );
			...
			if ( is_file( $template_path ) ) {
				...
				$template_content = apply_filters( "wpmolyp_{$admin}template_content", ob_get_clean(), $default_template_path, $template_path, $variables );
			}
			else
				$template_content = '';

			do_action( "wpmolyp_render_{$admin}template_after", $default_template_path, $variables, $template_path, $template_content );
